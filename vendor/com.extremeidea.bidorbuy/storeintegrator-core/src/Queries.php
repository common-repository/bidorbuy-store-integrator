...
class Queries
{
    ...
    public function getAddJobQueries($productIds, $productStatus)
    {
        ...
        return $this->insert($this->tablePrefix . self::TABLE_BOBSI_TRADEFEED_AUDIT, array(
                'product_id',
                'product_status',
                'row_created_on',
                'row_updated_on'
    ...
    public function getClearJobsQueries($timeStart)
    {
        return array(//delete all jobs marked as DELETE
            'clear_finished_jobs' => $this->delete(
                $this->tablePrefix . self::TABLE_BOBSI_TRADEFEED_AUDIT,
            ...
            'return_unfinished_jobs' => $this->update(
                $this->tablePrefix . self::TABLE_BOBSI_TRADEFEED_AUDIT,
                array(
                    'product_status' => self::STATUS_UPDATE,
                    'row_updated_on' => date("Y-m-d H:i:s"),
                    'row_status' => self::STATUS_NEW
                ),
                "`row_status` = '" . self::STATUS_PROCESSING . "' AND `row_updated_on` < '"
                . date("Y-m-d H:i:s", $timeStart - self::PRODUCT_PROCESS_TIMEOUT) . "'"
            )
        );
    }
     ...
     * Set jobs
     *
     * @param string   $productsIds products ids
    ...
    {
        return $this->select(
            $this->tablePrefix . self::TABLE_BOBSI_TRADEFEED,
    ...
    public function getSelectUntouchedProducts($ids)
    {
        return $this->select(
            $this->tablePrefix . self::TABLE_BOBSI_TRADEFEED_AUDIT,
            '`product_id` as id',
            "`product_id` IN (" . $ids . ") AND `row_status` = '" . self::STATUS_NEW . "'",
            true
        );
    }
    ...
    public function getXmlViewDataQuery($categoryIds)
    {
        ...
        foreach ($categoryIds as $categoryId) {
            $whereCategories[] = "t.`category_id` LIKE '%" . Tradefeed::CATEGORY_ID_DELIMITER . $categoryId
                . Tradefeed::CATEGORY_ID_DELIMITER . "%'";
        }
        $query = array();
        ...
        $query[] = "IF(LENGTH(t.`category`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_CATEGORY,
                "', t.`category`, '",
                true,
                3
            )
        ...
        $query[] = "'"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_PRICE,
                "', REPLACE(FORMAT(t.`price`, 2), ',', ''), '",
                true,
                3
            )
            . "'";
        $query[] = "IF(t.`market_price` > 0, CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_MARKET_PRICE,
                "', REPLACE(FORMAT(t.`market_price`, 2), ',', ''), '",
                true,
                3
            )
            . "'), '')";
        ...
        $query[] = "'"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_CONDITION,
                "', t.`condition`, '",
                true,
                3
            )
            . "'";
        $query[] = "IF(LENGTH(t.`image_url`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_IMAGE_URL,
                "', t.`image_url`, '",
                true,
                3
            )
            . "'), '')";
        $query[] = "IF(LENGTH(t.`images`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_IMAGES,
                "', t.`images`, '",
                false,
                3,
                true
            ...
            . "'), '')";
        $query[] = "IF(LENGTH(au.`summary`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_SUMMARY,
                "', au.`summary`, '",
                true,
                3
            )
            . "'), '')";
        $query[] = "IF(LENGTH(au.`description`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_DESCRIPTION,
                "', au.`description`, '",
                true,
                3
            )
        ...
        $query[] = "IF(LENGTH(t.`shipping_product_class`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_SHIPPING_CLASS,
                "', t.`shipping_product_class`, '",
                true,
                3
            )
            . "'), '')";
        $query[] = "IF(LENGTH(t.`attr_custom_attrs`), CONCAT('"
            . $this->tradefeed->section(
                Tradefeed::NAME_PRODUCT_ATTRIBUTES,
                "', t.`attr_custom_attrs`, '",
                false,
                3,
                true
        ...
        $query = $this->tradefeed->section(Tradefeed::NAME_PRODUCT, join(',', $query), false, 2);
